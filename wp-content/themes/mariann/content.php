...
$post_loop_details = mariann_get_post_details();
...
if ( defined('DEMO_MODE') && isset($_GET['blog_post_show_author']) ) {
  if($_GET['blog_post_show_author'] == 1) {
    $mariann_theme_options['blog_post_show_author'] = true;
  }
  if($_GET['blog_post_show_author'] == 0) {
    $mariann_theme_options['blog_post_show_author'] = false;
  }
}
...
if($current_post_format == 'format-gallery') {
	...
	// Default layout
	$blog_thumb_size = 'mariann-blog-thumb';

	// 2 column layout
	if($blog_enable_2column_design && $span_class == 'col-md-12') {
		$blog_thumb_size = 'mariann-blog-thumb-2column';
	}

	if($blog_enable_2column_design && $span_class == 'col-md-8') {
		$blog_thumb_size = 'mariann-blog-thumb-2column-sidebar';
	}
	...
	if($blog_enable_2column_design && $current_post_2column_advanced && $span_class == 'col-md-8') {
		$blog_thumb_size = 'mariann-blog-thumb-sidebar';
	}
	
	$gallery_images_data = mariann_cmb2_get_images_src( get_the_ID(), '_mariann_gallery_file_list', $blog_thumb_size );
...
if(($post_loop_id - 1 == floor(get_option('posts_per_page')/2))&&(!$blog_enable_2column_design)&&(!$blog_enable_masonry_design)) {
	mariann_banner_show('posts_loop_middle');
}
		...
		<div class="post-content-wrapper"<?php if(($current_post_sticky)&&($blog_enable_masonry_design)&&($image_bg !== '')&&(!in_array($current_post_format, $post_formats_media))) { echo ' data-style="'.esc_attr($image_bg).'"'; } ?>>
			...
			if($blog_enable_masonry_design) {
				...
				<div class="blog-post-thumb">
					...
					<?php the_post_thumbnail('mariann-blog-thumb-masonry-sidebar'); ?>
					</a>
				</div>
				<?php
				endif;
			...
			<div class="post-content">
				...
				<div class="post-categories"><?php printf( esc_html__( '%1$s', 'mariann' ), $categories_list ); ?></div>
				...
				<?php if(isset($mariann_theme_options['blog_post_show_author'])&&($mariann_theme_options['blog_post_show_author'])): ?>
				<div class="post-subtitle-container">
					<div class="post-author"><?php esc_html_e('By','mariann'); ?><div class="post-author-avatar"><?php echo get_avatar( get_the_author_meta('email'), '25' ); ?></div><?php the_author_posts_link();?></div>
				</div>
				<?php endif; ?>

				<?php
				// Don't show short content if user disabled it
				if((isset($mariann_theme_options['blog_post_loop_type']) && $mariann_theme_options['blog_post_loop_type']!=='none') || $blog_enable_masonry_design):?>
				<div class="entry-content">
					...
					if($blog_enable_masonry_design) {
						...
						if(isset($mariann_theme_options['blog_post_loop_type']) && $mariann_theme_options['blog_post_loop_type']=='excerpt') {
							if(in_array($current_post_format, $post_formats_hidereadmore)) {
								the_content(esc_html__('Leia mais', 'mariann'));
							} else {
								the_excerpt();
								?>
								<a href="<?php the_permalink(); ?>" class="more-link btn alt"><?php esc_html_e('Leia mais', 'mariann'); ?></a>
								<?php
							}
						}

						// Post loop type content
						if(isset($mariann_theme_options['blog_post_loop_type']) && $mariann_theme_options['blog_post_loop_type']=='content') {
...
', '<i class="fa fa-comment" aria-hidden="true"></i>1', '<i class="fa fa-comment" aria-hidden="true"></i>%'); ?></div>
				...
				<?php if(isset($mariann_theme_options['blog_list_show_share'])&&($mariann_theme_options['blog_list_show_share'])): ?>
	...
	<?php if(isset($mariann_theme_options['blog_list_show_related'])&&($mariann_theme_options['blog_list_show_related'])&&!$blog_enable_masonry_design&&!$blog_enable_layout_text&&!$blog_enable_2column_design): ?>
