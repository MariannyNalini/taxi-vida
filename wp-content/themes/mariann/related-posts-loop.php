...
if(!isset($mariann_theme_options['blog_list_related_by'])) {
	$mariann_theme_options['blog_list_related_by'] = 'tags';
}

$args = '';

// If by tags
if($mariann_theme_options['blog_list_related_by'] == 'tags' && $tags) {
...
// If by categories
if($mariann_theme_options['blog_list_related_by'] == 'cats' && $cats) {
...
if( $my_query->have_posts() ) {
	...
	echo '<h5>'.esc_html__('You may also like','mariann').'</h5>';

	while ($my_query->have_posts()) : $my_query->the_post(); 
		$post_image_data = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'mariann-blog-thumb');
