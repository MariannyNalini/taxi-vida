...
if(!isset($mariann_theme_options['blog_post_hide_featured_image'])) {
	$mariann_theme_options['blog_post_hide_featured_image'] = false;
} 
...
if ( defined('DEMO_MODE') && isset($_GET['post_sidebar_position']) ) {
  $mariann_theme_options['post_sidebar_position'] = $_GET['post_sidebar_position'];
}

if(!isset($mariann_theme_options['post_sidebar_position'])) {
	$mariann_theme_options['post_sidebar_position'] = 'disable';
}
...
if($post_sidebarposition == "0") {
	$post_sidebarposition = $mariann_theme_options['post_sidebar_position'];
}
...
// Post media
$post_embed_video = get_post_meta( get_the_ID(), '_mariann_video_embed', true );
...
$post_embed_audio = get_post_meta( get_the_ID(), '_mariann_audio_embed', true );
...
if($current_post_format == 'format-gallery') {
	...
	$gallery_images_data = mariann_cmb2_get_images_src( get_the_ID(), '_mariann_gallery_file_list', 'mariann-blog-thumb' );
...
$header_background_image = get_post_meta( get_the_ID(), '_mariann_header_image', true );
...
<div class="container-fluid container-page-item-title<?php echo esc_attr($header_background_class); ?>" data-style="<?php echo esc_attr($header_background_image_style); ?>">
	...
	<div class="page-item-title-single">
	    ...
	    <?php if(isset($mariann_theme_options['blog_enable_singlepost_header_info'])&&($mariann_theme_options['blog_enable_singlepost_header_info'])): ?>
	    <div class="post-info clearfix">
			<?php if(!isset($post_socialshare_disable) || !$post_socialshare_disable): ?>
			<div class="post-info-share">
				<?php do_action('mariann_social_share'); // this action called from plugin ?>
			</div>
			<?php endif; ?>
		</div>
		<?php endif; ?>
...
<?php if ( is_active_sidebar( 'post-sidebar' ) && ( $post_sidebarposition == 'left')) : ?>
		...
		<div class="<?php echo esc_attr($span_class); ?>">
			<div class="blog-post blog-post-single clearfix">
				<?php if(isset($mariann_theme_options['blog_enable_author_info_vertical'])&&($mariann_theme_options['blog_enable_author_info_vertical'])): ?>
				<div class="post-info-vertical">
					...
					<?php comments_popup_link( __( '0 Comments', 'mariann' ), __( '1 Comment', 'mariann' ), __( '% Comments', 'mariann' ) ); ?>
					</div>
					<?php endif; ?>
					<?php if(isset($mariann_theme_options['blog_enable_post_views_counter'])&&($mariann_theme_options['blog_enable_post_views_counter'])): ?>
					<div class="post-info-vertical-views"><?php do_action('mariann_post_views_text'); // this action called from plugin ?> <?php esc_html_e('Views', 'mariann'); ?></div>
					<?php endif; ?>
					
					<?php if(isset($mariann_theme_options['blog_post_show_share'])&&($mariann_theme_options['blog_post_show_share'])): ?>
					<?php if(!isset($post_socialshare_disable) || !$post_socialshare_disable): ?>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					<div class="post-content-wrapper">
						...
						<div class="post-content">
							<?php 
							if ( has_post_thumbnail()&&!in_array($current_post_format, $post_formats_media)&&(!$post_disable_featured_image ) && (!$mariann_theme_options['blog_post_hide_featured_image']) ): // check if the post has a Post Thumbnail assigned to it.
							?>
							<div class="blog-post-thumb">
							
								<?php the_post_thumbnail('mariann-blog-thumb'); ?>
							...
							mariann_banner_show('single_post_top');
							?>							
							...
							<div class="entry-content">
								<?php the_content('<div class="more-link">'.esc_html__( 'Continue reading...', 'mariann' ).'</div>' ); ?>
								<?php
									wp_link_pages( array(
										'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'mariann' ),
										'after'  => '</div>',
									) );
								?>
							</div><!-- .entry-content -->
							...
							mariann_banner_show('single_post_bottom');
							?>
				...
				<?php if(isset($mariann_theme_options['blog_enable_post_info_bottom'])&&($mariann_theme_options['blog_enable_post_info_bottom'])): ?>
				<div class="post-info clearfix">
					<?php if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) : ?>
					<div class="post-info-comments"><?php comments_popup_link( __( '<i class="fa fa-comment-o" aria-hidden="true"></i>
Deixar um comentário', 'mariann' ), '<i class="fa fa-comment" aria-hidden="true"></i>1', '<i class="fa fa-comment" aria-hidden="true"></i>%' ); ?></div>
					<?php endif; ?>

					<?php if(isset($mariann_theme_options['blog_enable_post_views_counter'])&&($mariann_theme_options['blog_enable_post_views_counter'])): ?>
					<div class="post-info-views"><?php do_action('mariann_post_views'); // this action called from plugin ?></div>
					<?php endif; ?>
					<?php if(isset($mariann_theme_options['blog_post_show_share'])&&($mariann_theme_options['blog_post_show_share'])): ?>
					<?php if(!isset($post_socialshare_disable) || !$post_socialshare_disable): ?>
					<div class="post-info-share">
			...
			<?php if(isset($mariann_theme_options['blog_enable_author_info'])&&($mariann_theme_options['blog_enable_author_info'])): ?>
			...
			<?php 
			if(isset($mariann_theme_options['blog_post_navigation']) && $mariann_theme_options['blog_post_navigation']) {
				mariann_content_nav( 'nav-below' ); 
			}
			?>

			<?php if(isset($mariann_theme_options['blog_post_show_related'])&&($mariann_theme_options['blog_post_show_related'])): ?>
