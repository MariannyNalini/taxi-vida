    ...
    class IPANEL
    {
        ...
        public function enqueue()
        {
            if ((isset($_GET['page'])) && ('ipanel_' . $this->args['ID'] == $_GET['page'])) {
                $assets_url = MARIANN_IPANEL_URI . 'assets/';
                ...
                wp_localize_script(
                    ...
                    array(
                        ...
                        'reset_confirm_message' => esc_html__("Are you sure you want to reset ALL theme options back to the defaults? You can't revert it back if you don't made backup.", 'mariann'),
                        'confirm_message' => esc_html__("Are you sure?", 'mariann'),
        ...
        function saveOptions($data, $panel_id)
        {
            ...
            $output['message'] = $output['status'] === 'error' ? esc_html__('Error!', 'mariann') : esc_html__('Options Updated.', 'mariann');
            $output['test'] = ($data);
        ...
        function ajaxRequestsHandle()
        {
            ...
            if ($_nonce === false) {
                die(
                    json_encode(
                        array(
                        'status' => 'error',
                        'message' => esc_html__('Error!', 'mariann')
                        )
                    )
                );
            }
            if ($_POST['action2'] == 'save_settings') {
                ...
                if ($_POST['import_type'] == 'code') { // Check if import data is code
                    ...
                    die(
                        json_decode(
                            array(
                            'status' => 'error',
                            'message' => esc_html__("You've not specified any data.", 'mariann')
                            )
                        )
                    );
                }
                ...
                if ($add_option === true) { // Check if add data was successfuly
                    ...
                    $output['message'] = esc_html__('Data has been imported successfully.', 'mariann');
                } else { // and if not
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('An error occured while importing data.', 'mariann');
                }
                ...
                $output['message'] = $delete === false ? esc_html__('Error!', 'mariann') : esc_html__('Options Reset.', 'mariann');
                $this->saveAllOptions();
                ...
                if (@unlink(urldecode($_POST['file_path']))) {
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('The file permanently deleted.', 'mariann');
                } else {
                    $output['status']  = 'error';
                    $output['message'] = esc_html__('ERROR!', 'mariann');
                }
                echo json_encode($output);
                ...
                if (current_user_can('upload_files')) {
                    ...
                    if (array_key_exists('error', $movefile)) {
                        ...
                        if ($is_image) {
                            ...
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'mariann').': ' . '<code>' . $movefile['url'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'mariann').': ' . '<code>' . $movefile['file'] . '</code><br/><br/>' . "\n";
                            ...
                            $HTMLRESULTS = '    <img class="ipanel-qup-up-img" src="' . MARIANN_IPANEL_URI . 'assets/img/document.png" />' . "\n";
                            $HTMLRESULTS .= '    <div class="ipanel_file_info">' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'mariann').': ' . '<code>' . $movefile['url'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'mariann').': ' . '<code>' . $movefile['file'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . $movefile['file'] . '">Delete</a>' . "\n";
        ...
        public function addOptions()
        {
            if (get_option($this->args['ID']) === false) {
                if (defined('MARIANN_IPANEL_PLUGIN_USAGE') && MARIANN_IPANEL_PLUGIN_USAGE !== true) {
        ...
        public function downloadExport()
        {
            if (isset($_GET['action']) && $_GET['action'] == 'ipanel-download-export-file') {
                ...
                if ($data === false) {
                    wp_die(esc_html__('Error in download the file.', 'mariann'));
                }
        ...
        public function menuPage()
        {
            ...
            <div class="ipanel-form-stuff" style="margin-top: 20px;">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo esc_html__('Save All Changes', 'mariann'); ?></button>
                ...
                <a class="button button-primary ipanel-more-themes" target="_blank" href="http://creanncy.com/themes/?from=mariann_cp"><?php echo esc_html__('More themes', 'mariann'); ?></a>
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo esc_html__('Reset', 'mariann'); ?></button>
                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.esc_html__('Toggle Preview', 'mariann').'</button>'; 
                    } 
                ?>
                </div>
                <div class="clr"></div>
            ...
            <div id="ipanel-preview-wrap" style="display:none;">
                ...
                <div class="imac ipanel-device" id="ipanel-preview-desktop">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="1010" height="573"><?php echo esc_html__("Your browser doesn't support iframes.", 'mariann'); ?></iframe>
                </div>
                
                <div class="ipad ipanel-device" id="ipanel-preview-tablet">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="767" height="1037"><?php echo esc_html__("Your browser doesn't support iframes.", 'mariann'); ?></iframe>
                </div>
                
                <div class="iphone ipanel-device" id="ipanel-preview-phone">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="642" height="1127"><?php echo esc_html__("Your browser doesn't support iframes.", 'mariann'); ?></iframe>
                </div>
            ...
            <div class="ipanel-form-stuff">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo esc_html__('Save All Changes', 'mariann'); ?></button>
                <div class="theme-copyright">Theme developed by <a href="http://creanncy.com/" target="_blank">Creanncy Company</a>. Powered by <a href="http://creanncy.com/" target="_blank">Creanncy Framework</a>.</div>
                <div class="buttons-panel-right">
                <a class="button button-primary ipanel-more-themes" target="_blank" href="http://creanncy.com/themes/"><?php echo esc_html__('More themes', 'mariann'); ?></a>
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo esc_html__('Reset', 'mariann'); ?></button>
                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.esc_html__('Toggle Preview', 'mariann').'</button>'; 
                    } 
        ...
        { 
            if(isset($field_options['repeater_item'])) {
        ...
        public function quickUpload($options = null, $Repeater = array())
        {
            ...
            if (!empty($options['field_options']['btn_text'])) {
                ...
                $btn_text = esc_html__("Choose File", 'mariann');
            }
            ...
            if (filter_var($value['url'], FILTER_VALIDATE_URL) !== false) {
                ...
                if ($is_image) {
                    ...
                    $input .= '    '.esc_html__('File URL', 'mariann').': ' . '<code>' . $value['url'] . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'mariann').': ' . '<code>' . $value['path'] . '</code><br/><br/>' . "\n";
                    $input .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . $value['path'] . '">'.esc_html__('Delete', 'mariann').'</a>' . "\n";
                    $input .= '    </div>' . "\n";
                    $input .= '    <div class="clr"></div></div>' . "\n";
                } else {
                    // File Box for Other File Types
                    $input .= '    <div class="ipanel-quick-upload-file">' . "\n";
                    $input .= '    <img class="ipanel-qup-up-img" src="' . MARIANN_IPANEL_URI . 'assets/img/document.png" />' . "\n";
                    $input .= '    <div class="ipanel_file_info">' . "\n";
                    $input .= '    '.esc_html__('File URL', 'mariann').': ' . '<code>' . $value['url'] . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'mariann').': ' . '<code>' . $value['path'] . '</code><br/><br/>' . "\n";
        ...
        public function repeater($options = null)
        {
            ...
            if (count($value)) { // If repeater has any item
                foreach ($value as $itemKey => $itemVal) {
                    ...
                    $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'mariann') . '<span class="item_number"> ' . ($i + 1) . '</span>' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '<span class="item_number">' . $i + 1 . '</span>') . '</h2>';
                ...
                $inputs .= '<i class="ipanel-delete-icon ipanel-delete-repeater-item"></i>';
                $inputs .= '<i class="ipanel-drag-icon ipanel-drag-repeater-item"></i>';
                $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'mariann') . ' 1' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '1') . '</h2>';
                $inputs .= '<div class="ipanel-repeater-fields">';
            ...
            $inputs .= '<button class="ipanel-button ipanel-button-white ipanel-repeater-new-item">'.esc_html__("New Item", 'mariann').'</button>';
        ...
        {
            $button_text  = isset($options['field_options']['button_text']) ? $options['field_options']['button_text'] : esc_html__("Upload", 'mariann');
            $media_title  = isset($options['field_options']['media_title']) ? $options['field_options']['media_title'] : esc_html__("Choose File", 'mariann');
            $media_button = isset($options['field_options']['media_insert_button_text']) ? $options['field_options']['media_insert_button_text'] : esc_html__("Choose File", 'mariann');
            if (isset($options['repeater_item']) && $options['repeater_item'] === true) {
                $option_name = $Repeater['id'] . '[' . $Repeater['pos'] . '][' . $options['id'] . ']';
            } else {
                $option_name = $options['id'];
        ...
        public function export($options = null)
        {
            ...
            if ($export_value === false) {
                $export_value = esc_html__("You need to save options first.", 'mariann');
            }
            $op = '';
            ...
            $op .= '<a href="' . esc_url($url) . '" class="ipanel-button ipanel-button-white ipanel-export-link-btn" target="_blank"> ' . esc_html__('Download Theme Options Backup File', 'mariann') . '</a>';
            $op .= '</div>';
        ...
        public function import($options = null)
        {
            ...
            $op .= '<label><p>' . esc_html__('Paste exported theme options string to import options from backup:', 'mariann') . '</p><textarea class="ipanel-import-text"></textarea></label>';
            $op .= '<br/><br/>OR import from file:<br/><br/><label class="ipanel-import-file-label ipanel-button ipanel-button-white"> ' . esc_html__('Choose Theme Options Backup File', 'mariann') . ' <input type="file" name="ipanel-import-file" class="ipanel-import-file"/></label>';
            $op .= '<span class="ipanel-chosen-filename"></span>';
            $op .= '<br/><br/><br/><a class="ipanel-button ipanel-button-white ipanel-import-button">'.esc_html__('Import options', 'mariann').'</a><br/><br/>';
            $op .= '</div>';
            return $this->_fieldSection($options, $op);
        }
